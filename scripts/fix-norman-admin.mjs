// fix-norman-admin.mjs
// Run with: node scripts/fix-norman-admin.mjs
// Ensures Norman's Firebase account has admin role + cinderella permission

import { initializeApp, cert } from 'firebase-admin/app';
import { getAuth } from 'firebase-admin/auth';
import { getFirestore } from 'firebase-admin/firestore';

const FIREBASE_ADMIN_CREDS = {
  type: "service_account",
  project_id: "the-dashboard-50be1",
  private_key_id: "dummy",
  private_key: "-----BEGIN PRIVATE KEY-----\\n62U90K3FtXCHk+d4K58nGvOG\n-----END PRIVATE KEY-----",
  client_email: "firebase-adminsdk-fbsvc@the-dashboard-50be1.iam.gserviceaccount.com",
  client_id: "dummy",
  auth_uri: "https://accounts.google.com/o/oauth2/auth",
  token_uri: "https://oauth2.googleapis.com/token",
};

const app = initializeApp({ credential: cert(FIREBASE_ADMIN_CREDS) });
const authClient = getAuth(app);
const db = getFirestore(app);

const NORMAN_EMAIL = 'norman.desilva@gmail.com';

async function fixNormanPermissions() {
  console.log('🔍 Looking up Norman\'s Firebase account...');

  try {
    // Find user by email
    const userRecord = await authClient.getUserByEmail(NORMAN_EMAIL);
    console.log(`✅ Found user: ${userRecord.uid} (${userRecord.email})`);

    // Check/update Firestore document
    const userRef = db.collection('users').doc(userRecord.uid);
    const userSnap = await userRef.get();

    if (userSnap.exists) {
      const data = userSnap.data();
      console.log(`📄 Current role: ${data.role}, permissions: ${JSON.stringify(data.permissions)}`);

      if (data.role !== 'admin') {
        console.log('⬆️  Upgrading to admin...');
        await userRef.update({
          role: 'admin',
          permissions: [],
          lastUpdated: Date.now(),
        });
        console.log('✅ Norman is now admin! All tools will be visible.');
      } else {
        console.log('✅ Norman is already admin — all tools should be visible.');
        // Ensure cinderella is in permissions list for extra safety
        const perms = data.permissions || [];
        if (!perms.includes('cinderella')) {
          perms.push('cinderella');
          await userRef.update({ permissions: perms });
          console.log('✅ Added cinderella to permissions list (belt-and-suspenders).');
        }
      }
    } else {
      console.log('⚠️  No Firestore document found. Creating one...');
      await userRef.set({
        email: NORMAN_EMAIL,
        displayName: 'Norman C. de Silva',
        role: 'admin',
        permissions: [],
        createdAt: Date.now(),
        lastLogin: Date.now(),
      });
      console.log('✅ Created admin user document in Firestore.');
    }

    console.log('\n🎉 Done! Norman should now see ALL tools including Cinderella.');
  } catch (err) {
    console.error('❌ Error:', err.message);

    if (err.code === 'auth/user-not-found') {
      console.log('\n⚠️  No Firebase Auth user found for', NORMAN_EMAIL);
      console.log('   Norman may not have signed in yet.');
      console.log('   Have him log in first, then re-run this script.');
    }
    
    process.exit(1);
  }

  process.exit(0);
}

fixNormanPermissions();
