...
async function readLegalDocs() {
  try {
    console.log('📋 Querying jimmy_deliverables where createdBy = legal-jimmy (no orderBy)...\n');
    
    const snapshot = await db.collection('jimmy_deliverables')
    ...
    if (snapshot.size === 0) {
      console.log('No documents found. The document may not have been saved correctly.');
    }
    ...
    snapshot.forEach(doc => {
      ...
      console.log(`Preview: ${(data.preview || '').substring(0, 150)}...`);
