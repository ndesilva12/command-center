#!/usr/bin/env node
/**
 * Clear old War Room data from Firestore
 * Run with: node scripts/clear-warroom-data.js
 */

const admin = require('firebase-admin');

// Initialize Firebase Admin
const FIREBASE_ADMIN_CREDS = {
  type: "service_account",
  project_id: "the-dashboard-50be1",
  private_key: "-----BEGIN PRIVATE KEY-----\\n62U90K3FtXCHk+d4K58nGvOG\n-----END PRIVATE KEY-----",
  client_email: "firebase-adminsdk-fbsvc@the-dashboard-50be1.iam.gserviceaccount.com",
};

if (!admin.apps.length) {
  admin.initializeApp({
    credential: admin.credential.cert(FIREBASE_ADMIN_CREDS),
  });
}

const db = admin.firestore();

// Old CC War Room collections to clear
const OLD_COLLECTIONS = [
  'cinderella_roster_configs', // Old budget configs
  'cinderella_network',        // Old network data
  'cinderella_depth_charts',   // Old depth charts (if any)
];

async function clearCollection(collectionName) {
  try {
    const snapshot = await db.collection(collectionName).get();
    if (snapshot.empty) {
      console.log(`  ${collectionName}: No documents found (already empty)`);
      return 0;
    }

    const batch = db.batch();
    let count = 0;
    
    snapshot.forEach(doc => {
      batch.delete(doc.ref);
      count++;
    });

    await batch.commit();
    console.log(`  ${collectionName}: Deleted ${count} documents`);
    return count;
  } catch (error) {
    console.log(`  ${collectionName}: Error - ${error.message}`);
    return 0;
  }
}

async function main() {
  console.log('\n🗑️  Clearing old CC War Room data from Firestore...\n');
  
  let totalDeleted = 0;
  
  for (const collection of OLD_COLLECTIONS) {
    totalDeleted += await clearCollection(collection);
  }
  
  console.log(`\n✅ Done! Cleared ${totalDeleted} total documents.`);
  console.log('\nMaking Cinderella will use fresh collections:');
  console.log('  - mc_bigboard');
  console.log('  - mc_depth_charts');
  console.log('  - mc_budget_configs');
  console.log('  - mc_network');
  
  process.exit(0);
}

main();
