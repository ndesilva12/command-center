...
export async function POST(request: NextRequest) {
  try {
    ...
    const { company, query } = body;
    const topic = company || query;

    if (!topic || !topic.trim()) {
      return NextResponse.json({ error: 'Company or query is required' }, { status: 400 });
    }
    ...
    // Research the company/topic
    const searches = [
      `${topic} company overview business`,
      `${topic} news recent developments`,
      `${topic} competitors market analysis`,
      `${topic} leadership team executives`,
    ];
    ...
    const prompt = `Create a business intelligence brief on: "${topic}"
...
Return ONLY valid JSON:
{
  ...
  "sources": ["List of source URLs"]
    ...
    try {
      ...
      await adminDb.collection('business_history').add(response);
    } catch (dbError) {
